...
public:
    CollectSegs(const Vector & mouse, double snapDist, const Page * page, int view);
...
CollectSegs::CollectSegs(const Vector & mouse, double snapDist, const Page * page,
			 int view)
    : iMouse(mouse)
    , iDist(snapDist) {
	...
	for (int i = 0; i < page->count(); ++i) {
	    if (page->layerOf(i) == gridLayer) page->object(i)->accept(*this);
	}
    } else {
	for (int i = 0; i < page->count(); ++i) {
	    if (page->objSnapsInView(i, view)) page->object(i)->accept(*this);
	}
    }
}
...
void CollectSegs::visitPath(const Path * obj) {
	...
	case SubPath::ECurve: {
		...
		case CurveSegment::EArc:
		    arc = m * seg.arc();
		    if (arc.distance(iMouse, iDist) < iDist) iArcs.push_back(arc);
		    break;
...
void Snap::intersectionSnap(const Vector & pos, Vector & fifi, const Page * page,
	...
	for (int j = i + 1; j < size(segs.iSegs); ++j) {
	    if (segs.iSegs[i].intersects(segs.iSegs[j], v)) pts.push_back(v);
	}
    }
...
bool Snap::snapAngularIntersection(Vector & pos, const Line & l, const Page * page,
				   int view, double snapDist) const noexcept {
    CollectSegs segs(pos, snapDist, page, view);
	...
	if (it->intersects(l, v)) pts.push_back(v);
    }
    for (std::vector<Arc>::const_iterator it = segs.iArcs.begin(); it != segs.iArcs.end();
	 ++it) {
	it->intersect(l, pts);
    }
...
Snap::TSnapModes Snap::simpleSnap(Vector & pos, const Page * page, int view,
    ...
    if (iSnap & ESnapVtx) {
	for (int i = 0; i < page->count(); ++i) {
	    if (page->objSnapsInView(i, view)) page->snapVtx(i, pos, fifi, d);
	}
	if (tool) tool->snapVtx(pos, fifi, d, false);
    }
    ...
    if (iSnap & ESnapCtl) {
	for (int i = 0; i < page->count(); ++i) {
	    if (page->objSnapsInView(i, view)) page->snapCtl(i, pos, fifiCtl, d);
	}
	if (tool) tool->snapVtx(pos, fifiCtl, d, true);
    }

    double dctl = d;
    Vector fifiX = pos;
    if (iSnap & ESnapInt) intersectionSnap(pos, fifiX, page, view, d);
    ...
    if (iSnap & ESnapBd) {
	for (int i = 0; i < page->count(); ++i) {
	    if (page->objSnapsInView(i, view)) page->snapBnd(i, pos, fifi, d);
	}
	if (d < snapDist) {
	    pos = fifi;
	    return ESnapBd;
...
*/
Snap::TSnapModes Snap::snap(Vector & pos, const Page * page, int view, double snapDist,
			    Tool * tool, Vector * autoOrg) const noexcept {
	...
	pos = l.project(pos);
	if (iSnap & ESnapBd) snapAngularIntersection(pos, l, page, view, snapDist);
	return ESnapAngle;
    }
...
bool Snap::setEdge(const Vector & pos, const Page * page, int view) noexcept {
	...
	Vector dir = l.dir();
	if ((iOrigin - seg.iP).len() > (iOrigin - seg.iQ).len()) dir = -dir;
	iDir = dir.angle();
	return true;
	...
	double t;
	double bound = 2.0;
	if (!bez.snap(pos, t, iOrigin, bound)) return false;
