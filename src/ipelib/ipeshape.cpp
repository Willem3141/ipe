...
inline bool snapVertex(const Vector & mouse, const Vector & v, Vector & pos,
		       double & bound) {
    return v.snap(mouse, pos, bound);
}

inline void snapBezier(const Vector & mouse, const Bezier & bez, Vector & pos,
		       double & bound) {
    double t;
...
void CurveSegment::beziers(std::vector<Bezier> & bez) const {
    switch (type()) {
    case EOldSpline: Bezier::oldSpline(countCP(), cps(), bez); break;
    case ESpline: Bezier::spline(countCP(), cps(), bez); break;
    case ECardinalSpline: Bezier::cardinalSpline(countCP(), cps(), tension(), bez); break;
    case ESpiroSpline: {
...
void CurveSegment::draw(Painter & painter) const {
    switch (type()) {
    case ESegment: painter.lineTo(cp(1)); break;
    case EOldSpline:
    case ESpline:
    ...
    case EArc: painter.drawArc(arc()); break;
    }
}
...
void CurveSegment::addToBBox(Rect & box, const Matrix & m, bool cpf) const {
	...
	if (cpf) box.addPoint((m * matrix()).translation());
	break;
    case ESpline:
    case EOldSpline:
    case ECardinalSpline:
    case ESpiroSpline:
	if (cpf) {
	    for (int i = 0; i < countCP(); ++i) box.addPoint(m * cp(i));
	} else {
	    std::vector<Bezier> bez;
	    beziers(bez);
...
double CurveSegment::distance(const Vector & v, const Matrix & m, double bound) const {
    switch (type()) {
    case ESegment: return Segment(m * cp(0), m * cp(1)).distance(v, bound);
    case EArc: return (m * arc()).distance(v, bound);
    case EOldSpline:
    case ESpline:
    case ECardinalSpline:
    case ESpiroSpline: {
...
/*! The method assumes that the first control point has already been
  tested. */
void CurveSegment::snapVtx(const Vector & mouse, const Matrix & m, Vector & pos,
...
void CurveSegment::snapBnd(const Vector & mouse, const Matrix & m, Vector & pos,
			   double & bound) const {
    switch (type()) {
    case ESegment: Segment(m * cp(0), m * cp(1)).snap(mouse, pos, bound); break;
    case EArc: {
	Arc a = m * arc();
	Vector pos1;
	Angle angle;
...
void Curve::appendSpline(const std::vector<Vector> & v, CurveSegment::Type type) {
    assert(type == CurveSegment::ESpline || type == CurveSegment::ECardinalSpline
	   || type == CurveSegment::EOldSpline);
    if (iSeg.empty()) iCP.push_back(v[0]);
    assert(v[0] == iCP.back());
    for (int i = 1; i < size(v); ++i) iCP.push_back(v[i]);
    Seg seg;
    seg.iType = type;
    seg.iLastCP = iCP.size() - 1;
    seg.iMatrix = 0;
...
void Curve::appendSpiroSpline(const std::vector<Vector> & v) {
    ...
    for (int i = 0; i < size(v); ++i) iCP.push_back(v[i]);
    Seg seg;
...
void Curve::appendSpiroSplinePrecomputed(const std::vector<Vector> & v, int sep) {
    ...
    for (int i = 1; i < sep; ++i) iCP.push_back(v[i]);
    ...
    for (int i = sep; i < size(v); ++i) iCP.push_back(v[i]);
    Seg seg;
    seg.iType = CurveSegment::ESpiroSpline;
    seg.iLastCP = iCP.size() - 1;
    seg.iBezier = bezIndex;
...
void Curve::setClosed(bool closed) {
    ...
    iClosed = closed;
    if (iClosed) appendSegment(iCP.back(), iCP.front());
}

SubPath::Type Curve::type() const { return ECurve; }

const Curve * Curve::asCurve() const { return this; }
...
void Curve::save(Stream & stream) const {
	...
	case CurveSegment::EOldSpline:
	    while (vtx < it->iLastCP) stream << iCP[vtx++] << "\n";
	    stream << iCP[vtx++] << " s\n";
	    break;
	...
	case CurveSegment::ESpiroSpline: {
	    while (vtx < it->iBezier - 1) stream << iCP[vtx++] << "\n";
...
void Curve::draw(Painter & painter) const {
    ...
    for (int i = 0; i < countSegments(); ++i) segment(i).draw(painter);
    if (closed()) painter.closePath();
}

void Curve::addToBBox(Rect & box, const Matrix & m, bool cp) const {
    for (int i = 0; i < countSegments(); ++i) segment(i).addToBBox(box, m, cp);
}

double Curve::distance(const Vector & v, const Matrix & m, double bound) const {
    double d = bound;
...
void Curve::snapVtx(const Vector & mouse, const Matrix & m, Vector & pos, double & bound,
		    bool ctl) const {
    if (!ctl)
...
void Curve::snapBnd(const Vector & mouse, const Matrix & m, Vector & pos,
		    double & bound) const {
    snapVertex(mouse, m * segment(0).cp(0), pos, bound);
    for (int i = 0; i < countSegmentsClosing(); ++i)
...
bool SubPath::closed() const { return true; }
...
SubPath::Type Ellipse::type() const { return EEllipse; }

const Ellipse * Ellipse::asEllipse() const { return this; }

void Ellipse::save(Stream & stream) const { stream << matrix() << " e\n"; }

void Ellipse::draw(Painter & painter) const { painter.drawArc(Arc(iM)); }

void Ellipse::addToBBox(Rect & box, const Matrix & m, bool cp) const {
    box.addRect(Arc(m * iM).bbox());
}

double Ellipse::distance(const Vector & v, const Matrix & m, double bound) const {
    Arc arc(m * iM);
    return arc.distance(v, bound);
}

//! snaps to center of ellipse.
void Ellipse::snapVtx(const Vector & mouse, const Matrix & m, Vector & pos,
		      double & bound, bool ctl) const {
    if (ctl) snapVertex(mouse, (m * iM).translation(), pos, bound);
}

void Ellipse::snapBnd(const Vector & mouse, const Matrix & m, Vector & pos,
		      double & bound) const {
    Arc arc(m * iM);
    Vector pos1;
    Angle angle;
...
SubPath::Type ClosedSpline::type() const { return EClosedSpline; }

const ClosedSpline * ClosedSpline::asClosedSpline() const { return this; }
...
double ClosedSpline::distance(const Vector & v, const Matrix & m, double bound) const {
    std::vector<Bezier> bez;
    beziers(bez);
    double d = bound;
    double d1;
...
void ClosedSpline::snapVtx(const Vector & mouse, const Matrix & m, Vector & pos,
			   double & bound, bool ctl) const {
    if (ctl) {
...
void ClosedSpline::snapBnd(const Vector & mouse, const Matrix & m, Vector & pos,
			   double & bound) const {
    std::vector<Bezier> bez;
    beziers(bez);
    for (const auto & b : bez) snapBezier(mouse, m * b, pos, bound);
...
Shape::Shape(const Vector & center, double radius) {
    ...
    appendSubPath(new Ellipse(Matrix(radius, 0.0, 0.0, radius, center.x, center.y)));
}

//! Convenience function: create circular arc.
/*! If \a alpha1 is larger than \a alpha0, the arc is oriented positively,
  otherwise negatively. */
Shape::Shape(const Vector & center, double radius, double alpha0, double alpha1) {
    iImp = new Imp;
    iImp->iRefCount = 1;
...
bool Shape::isSegment() const {
    if (countSubPaths() != 1) return false;
    const SubPath * p = subPath(0);
    if (p->type() != SubPath::ECurve || p->closed()) return false;
    const Curve * c = p->asCurve();
    if (c->countSegments() != 1 || c->segment(0).type() != CurveSegment::ESegment)
	return false;
    return true;
}
...
void Shape::addToBBox(Rect & box, const Matrix & m, bool cp) const {
    for (int i = 0; i < countSubPaths(); ++i) subPath(i)->addToBBox(box, m, cp);
}
...
void Shape::snapVtx(const Vector & mouse, const Matrix & m, Vector & pos, double & bound,
		    bool ctl) const {
    for (int i = 0; i < countSubPaths(); ++i)
	subPath(i)->snapVtx(mouse, m, pos, bound, ctl);
}

void Shape::snapBnd(const Vector & mouse, const Matrix & m, Vector & pos,
		    double & bound) const {
    for (int i = 0; i < countSubPaths(); ++i) subPath(i)->snapBnd(mouse, m, pos, bound);
}
...
void Shape::draw(Painter & painter) const {
    for (int i = 0; i < countSubPaths(); ++i) subPath(i)->draw(painter);
}

Shape::Imp::~Imp() {
    // delete the subpaths
    for (SubPathSeq::iterator it = iSubPaths.begin(); it != iSubPaths.end(); ++it) {
	delete *it;
	*it = nullptr;
    }
}
...
static Matrix getMatrix(std::vector<double> & args) {
    Matrix m;
    for (int i = 0; i < 6; ++i) m.a[i] = args[i];
...
void Shape::save(Stream & stream) const {
    for (int i = 0; i < countSubPaths(); ++i) subPath(i)->save(stream);
}
...
bool Shape::load(String data) {
	...
	if (stream.token() == "h") { // closing path
	    ...
	    if (!sp || args.size() != 2) return false;
	    stream.nextToken(); // eat token
	    while (!args.empty()) {
	    ...
	    if (!sp || args.size() != 8) return false;
	    stream.nextToken();
	    ...
	    org = v1;
	    mid = -1;
	} else if (stream.token() == "s" || stream.token() == "q" || stream.token() == "c"
		   || stream.token() == "C" || stream.token() == "L") {
	    size_t parity = (stream.token() == "C") ? 1 : 0;
	    if (!sp || args.size() < 2 || (args.size() % 2 != parity)) return false;
	    String typeToken = stream.token();
	    stream.nextToken();
	    std::vector<Vector> v;
	    v.push_back(org);
	    while (args.size() >= 2) v.push_back(getVector(args));
	    if (typeToken == "s")
    ...
    if (sbn > 0 && subPath(sbn - 1)->asCurve()
	&& subPath(sbn - 1)->asCurve()->countSegments() == 0)
