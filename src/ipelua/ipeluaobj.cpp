...
static const char * const type_names[] = {"group", "path",      "text",
					  "image", "reference", nullptr};

static const char * const pinned_names[] = {"none", "horizontal", "vertical", "fixed",
					    nullptr};

static const char * const pathmode_names[] = {"stroked", "strokedfilled", "filled",
					      nullptr};

static const char * const transformation_names[] = {"translations", "rigid", "affine",
						    nullptr};

const char * const ipelua::horizontal_alignment_names[] = {"left", "right", "hcenter",
							   nullptr};

const char * const ipelua::vertical_alignment_names[] = {"bottom", "baseline", "top",
							 "vcenter", nullptr};

const char * const ipelua::linejoin_names[] = {"normal", "miter", "round", "bevel",
					       nullptr};

const char * const ipelua::linecap_names[] = {"normal", "butt", "round", "square",
					      nullptr};

const char * const ipelua::fillrule_names[] = {"normal", "wind", "evenodd", nullptr};
...
static const char * const splinetype_names[] = {"bspline", "cardinal", "spiro"};
...
Attribute ipelua::check_bool_attribute(lua_State * L, int i) {
    ...
    if (lua_type(L, i) == LUA_TBOOLEAN) return Attribute::Boolean(lua_toboolean(L, i));
...
Attribute ipelua::check_property(Property prop, lua_State * L, int i) {
    ...
    case EPropTextSize: return check_number_attribute(L, i);
    ...
    case EPropRArrow:
    case EPropMinipage:
    case EPropTransformableText: return check_bool_attribute(L, i);
    }
    return Attribute::NORMAL(); // placate compiler
}

static void get_attribute(lua_State * L, int i, Property prop, const char * key,
			  Attribute & att) {
...
int ipelua::group_constructor(lua_State * L) {
    ...
    for (int i = 1; i <= no; ++i) {
	lua_rawgeti(L, 1, i);
	luaL_argcheck(L, is_type(L, -1, "Ipe.object"), 1, "element is not an Ipe object");
...
int ipelua::xml_constructor(lua_State * L) {
	...
	XmlAttributes attr;
	if (!parser.parseAttributes(attr)) return 0;
	tag = parser.parseToTag();

	while (tag == "bitmap") {
	    if (!parser.parseBitmap()) return false;
	    tag = parser.parseToTag();
	}

	for (;;) {
	    if (tag == "/ipeselection") return 2;
	    String layer;
...
static int object_tostring(lua_State * L) {
    ...
    lua_pushfstring(L, "Object(%s)@%p", type_names[s->obj->type()], lua_topointer(L, 1));
    return 1;
}
...
static int object_position(lua_State * L) {
    ...
    luaL_argcheck(L, obj->type() == Object::EText || obj->type() == Object::EReference, 1,
...
static int object_addToBBox(lua_State * L) {
    ...
    bool cp = true;
    if (lua_type(L, 4) == LUA_TBOOLEAN) cp = lua_toboolean(L, 4);
...
static bool collect_cp(lua_State * L, std::vector<Vector> & cp) {
	...
	if (!is_type(L, -1, "Ipe.vector")) return false;
...
static SubPath * get_ellipse(lua_State * L, int index) {
    ...
    if (!is_type(L, -1, "Ipe.matrix")) luaL_error(L, "element %d has no matrix", index);
...
static SubPath * get_curve(lua_State * L, int index) {
    ...
    if (!lua_isboolean(L, -1)) luaL_error(L, "element %d has no 'closed' field", index);
	...
	std::vector<Vector> cp;
	if (!collect_cp(L, cp))
	    luaL_error(L, "non-vector control point in segment %d of element %d", i + 1,
		       index);
	...
	case CurveSegment::ESegment: c->appendSegment(cp[0], cp[1]); break;
	case CurveSegment::ESpline: c->appendSpline(cp); break;
	case CurveSegment::EOldSpline: c->appendOldSpline(cp); break;
	...
	case CurveSegment::ESpiroSpline: c->appendSpiroSpline(cp); break;
	default: break;
	}
...
Shape ipelua::check_shape(lua_State * L, int index) {
	...
	if (!lua_istable(L, -1)) luaL_error(L, "element %d is not a table", i + 1);
	lua_getfield(L, -1, "type");
	// stack: subpath, type
	if (!lua_isstring(L, -1)) luaL_error(L, "element %d has no type", i + 1);
	int type = test_option(L, -1, subpath_names);
	lua_pop(L, 1); // type
	switch (type) {
	case SubPath::EEllipse: shape.appendSubPath(get_ellipse(L, i + 1)); break;
...
static int object_symbol(lua_State * L) {
    Object * s = check_object(L, 1)->obj;
    luaL_argcheck(L, s->type() == Object::EReference, 1, "not a reference object");
...
static const struct luaL_Reg object_methods[] = {{"__tostring", object_tostring},
