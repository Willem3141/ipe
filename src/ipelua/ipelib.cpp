...
static const char * const format_name[] = {"xml", "pdf", "unknown"};
...
static int document_constructor(lua_State * L) {
	...
	*d = Document::load(fname.z(), reason);
	...
	case Document::ENotAnIpeFile:
	    ...
	    break;
	default: lua_pushfstring(L, "Parsing error at position %d", reason); break;
	}
...
static int check_pageno(lua_State * L, int i, Document * d, int extra = 0) {
    ...
    luaL_argcheck(L, 1 <= n && n <= d->countPages() + extra, i, "invalid page number");
    return n - 1;
}
...
static int document_index(lua_State * L) {
	...
	if (!luaL_getmetafield(L, 1, key)) lua_pushnil(L);
    }
    return 1;
}
...
static uint32_t check_flags(lua_State * L, int index) {
    ...
    if (lua_toboolean(L, -1)) flags |= SaveFlag::Export;
    lua_pop(L, 1);
    lua_getfield(L, index, "nozip");
    if (lua_toboolean(L, -1)) flags |= SaveFlag::NoZip;
    lua_pop(L, 1);
    lua_getfield(L, index, "keepnotes");
    if (lua_toboolean(L, -1)) flags |= SaveFlag::KeepNotes;
    lua_pop(L, 1);
    lua_getfield(L, index, "markedview");
    if (lua_toboolean(L, -1)) flags |= SaveFlag::MarkedView;
    lua_pop(L, 1);
    return flags;
}
...
static int document_replaceSheets(lua_State * L) {
    ...
    if (!p->owned) sheets = new Cascade(*p->cascade);
...
static int document_completeLatexRun(lua_State * L) {
    ...
    if (converter == nullptr) luaL_error(L, "no Latex converter given");
    String log;
...
static const char * const tex_engine_names[] = {"default", "pdftex", "xetex", "luatex"};
...
static void propFlag(lua_State * L, const char * name, bool & flag) {
    ...
    if (!lua_isnil(L, -1)) flag = lua_toboolean(L, -1);
    lua_pop(L, 1);
}

static void propString(lua_State * L, const char * name, String & str) {
    lua_getfield(L, 2, name);
    if (lua_isstring(L, -1)) str = lua_tolstring(L, -1, nullptr);
    lua_pop(L, 1);
}
...
static int document_setProperties(lua_State * L) {
    ...
    for (int i = 0; i < 4; ++i) {
	if (!strcmp(tex.z(), tex_engine_names[i])) prop.iTexEngine = LatexType(i);
    }
...
static int file_format(lua_State * L) {
    ...
    if (!fd) luaL_error(L, "fopen error: %s", strerror(errno));
...
static int ipe_splinetobeziers(lua_State * L) {
    ...
    for (int i = 1; i <= no; ++i) {
	lua_rawgeti(L, 1, i);
	luaL_argcheck(L, is_type(L, -1, "Ipe.vector"), 1, "element is not a vector");
	...
	if (!lua_isstring(L, -1)) luaL_error(L, "spline has no type");
	int type = test_option(L, -1, segtype_names);
	if (type < CurveSegment::ESpline) luaL_error(L, "spline has invalid type");
	lua_pop(L, 1); // pop type
	switch (type) {
	case CurveSegment::ESpline: Bezier::spline(v.size(), v.data(), result); break;
	...
	case CurveSegment::ECardinalSpline: {
	    ...
	    if (!lua_isnumber(L, -1)) luaL_error(L, "spline has no tension");
...
#define l_checkmode(mode)                                                                \
    (*mode != '\0' && strchr("rwa", *(mode++)) != nullptr && (*mode != '+' || ++mode)    \
     &&                          /* skip if char is '+' */                               \
...
static int ipe_readImage(lua_State * L) {
    ...
    Bitmap bmp = fmt ? Bitmap::readJpeg(s.z(), dotsPerInch, errmsg)
